...
public class DropdownListView extends ListView implements OnScrollListener {
			...
			switch (event.getAction()) {
				...
				case MotionEvent.ACTION_MOVE:
					...
					if (state != REFRESHING && isRecored && state != LOADING) {
						...
						// 更新headView的size
						if (state == PULL_To_REFRESH) {
							headView.setPadding(0, -1 * headContentHeight + (tempY - (int) mStartY) / RATIO, 0, 0);
						}

						// 更新headView的paddingTop
						if (state == RELEASE_To_REFRESH) {
							headView.setPadding(0, (tempY - (int) mStartY) / RATIO - headContentHeight, 0, 0);
						}
					}
	...
	public void setOnRefreshListenerHead(OnRefreshListenerHeader refreshListenerHeader) {

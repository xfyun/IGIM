...
import com.iflytek.cloud.imcall.utils.UserFinder;
...
public class MultiCallActivity extends BaseActivity implements View.OnClickListener, CallStateListener {

    private static final String TAG = "MultiCallActivity";
    ...
    private HashMap<Integer, SoftReference<SurfaceView>> mUidViewList = new HashMap<>();
    ...
    private void initLocalVideo() {
        ...
        mUidViewList.put(0, new SoftReference<>(surfaceV));
        mVideoContainer.initViewContainer(getApplicationContext(), 0, mUidViewList);
    ...
    private void doRenderRemoteUi(final int uid) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if (mUidViewList.containsKey(uid)) {
                    return;
                }

                SurfaceView surfaceV = IMCallClient.createRendererView(getApplicationContext());
                mUidViewList.put(uid, new SoftReference<>(surfaceV));
    ...
    private void doHideTargetView(int targetUid, boolean hide) {
        ...
        mVideoContainer.notifyUiChanged(mUidViewList, targetUid, status, null);
    }
    ...
    private void doRemoveRemoteUi(final int uid) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Object target = mUidViewList.remove(uid);
                if (target == null) {
                    return;
                }
                initVideoViewUi();
    ...
    public void onHangup(String userId) {
        Log.d(TAG, "onHangup: ");
        initUserIdUi();
        doRemoveRemoteUi(UserFinder.getAgoraid(userId, IMCallClient.getInstance().getUserList()));
    }
